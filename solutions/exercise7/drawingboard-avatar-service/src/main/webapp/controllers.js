...
function MainController($scope, DrawingService, $http) {
    ...
    if (!javaFXClient)
        $scope.eventSource = new EventSource("/drawingboard-avatar-service/api/events");
    ...
    if (!javaFXClient) {
        ...
        $scope.eventSource.addEventListener("message", eventHandler, false);
    }
...
function DrawingController($scope, $routeParams, DrawingService) {
    ...
    if (!javaFXClient) {
        $scope.websocket = new WebSocket("ws://" + document.location.host + "/drawingboard-avatar-service/websockets/" + $routeParams.drawingId);
        ...
        window.webSocketOpen.open("ws://" + document.location.host
                + "/drawingboard-avatar-service/websockets/", $routeParams.drawingId);
